...
export default function FileList({
  ...
  const handleStarFile = async (fileId: string) => {
    try {
      ...
      toast.error("Failed to star or unstar file. Please try again.");
    }
  };
  ...
  const handleTrashFile = async (fileId: string) => {
    try {
      ...
      const file = files.find((f) => f.id === fileId);
      if (file?.isTrash) {
        toast.success(`${file?.name} has been restored from trash`);
      } else {
        toast.success(`${file?.name} has been moved to trash`);
      }
    } catch (error) {
      console.error("Error trashing file:", error);
      toast.error("Failed to move file to trash. Please try again.");
    }
  };
  ...
  const handleEmptyTrash = async () => {
    try {
      ...
      // Show toast
      toast.success("Trash has been emptied successfully");
  ...
  return (
    <div className="space-y-6">
      ...
      {filteredFiles.length === 0 ? (
        ...
        <Card
          shadow="sm"
          className="border border-default-200 bg-default-50/80 backdrop-blur-md overflow-hidden"
        >
                  ...
                  >
                    <TableCell>
                      <div className="flex items-center gap-2 md:gap-3">
                        <FileIcon file={file} />
                        <div>
                          <div className="font-medium flex items-center gap-1 md:gap-2 text-default-800">
                            <span className="truncate max-w-[100px] sm:max-w-[160px] md:max-w-[220px] lg:max-w-[320px]">
                              {file.name}
                            </span>
                            {file.isStarred && (
